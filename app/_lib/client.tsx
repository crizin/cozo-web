...
export async function getBoards(): Promise<Response<Board[]>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/boards`, { next: { revalidate: 3600 } }).then((response) => validateResponse(response));
}
...
export async function getArticles(cursor = 0): Promise<Response<Page<Article[], number>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/articles?cursor=${cursor}`, { cache: 'no-store' }).then((response) => validateResponse(response));
}

export async function getBoardArticles(boardId: number, cursor = 0): Promise<Response<Page<Article[], number>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/boards/${boardId}/articles?cursor=${cursor}`, { cache: 'no-store' }).then((response) => validateResponse(response));
}

export async function getLinks(page = 1): Promise<Response<Page<Link[], number>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/links?page=${page}`, { cache: 'no-store' }).then((response) => validateResponse(response));
}

export async function getKeywordLastDate(): Promise<Response<string>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/keywords/last-date`, { cache: 'no-store' }).then((response) => validateResponse(response));
}

export async function getKeywords(date: string): Promise<Response<Page<TagTrend[], string>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/keywords/${date}`, { cache: 'no-store' }).then((response) => validateResponse(response));
}

export async function getSearchResults(keyword: string, page = 1): Promise<Response<Page<SearchResult, number>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/search?keyword=${encodeURIComponent(keyword)}&page=${page}`, { cache: 'no-store' }).then((response) =>
...
export async function getSearchResultsByBoard(boardId: number, keyword: string, page = 1): Promise<Response<Page<SearchResult, number>>> {
  return fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/search/board/${boardId}?keyword=${encodeURIComponent(keyword)}&page=${page}`, { cache: 'no-store' }).then(
